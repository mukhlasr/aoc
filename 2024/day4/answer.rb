...
diagonal2 = (0 ... n).map do |i|
  (0 .. i).map do |j|
    word_search[n - j].chars[n - i + j]
  end.join
end

# [[0,n]], [[0, n-1], [1, n]], [[0, n-2], [1, n-1], [2, n]], [[0, n-3], [1, n-2], [2, n-1], [3, n]], [[0, n-4], [1, n-3], [2, n-2], [3, n-1], [4, n]]
diagonal3 = (0 .. n).map do |i|
  (0 .. i).map do |j|
    word_search[j].chars[n - i + j]
  end.join
end

# [[n,0]], [[n-1, 0], [n, 1]], [[n-2, 0], [n-1, 1], [n, 2]], [[n-3, 0], [n-2, 1], [n-1, 2], [n, 3]], [[n-4, 0], [n-3, 1], [n-2, 2], [n-1, 3], [n, 4]]
diagonal4 = (0 ... n).map do |i|
  (0 .. i).map do |j|
    word_search[n - i + j].chars[j]
  end.join
end

part1 = [horizontal, vertical, diagonal1, diagonal2, diagonal3, diagonal4].map { |lines| lines.map { |line| countxmas(line) }.sum }.sum
